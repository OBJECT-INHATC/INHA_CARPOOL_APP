...
class ApiService {
  ...
  // 건의사항 저장
  Future<bool> saveSuggest(FeedbackRequestDTO feedbackRequestDTO) async {
    final String apiUrl = '$baseUrl/feedback/saveSuggestion';

    // ReportRequstDTO 객체를 JSON 문자열로 변환
    final String requestBody = jsonEncode(feedbackRequestDTO);
    print(requestBody);
    try {
      final response = await http.post(
        Uri.parse(apiUrl),
